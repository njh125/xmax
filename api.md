...
#### server
 ...
 1.构造请求授权地址,获取Authorization Code：
 ...
 | state|	是	|client端的状态值。用于第三方应用防止CSRF攻击，成功授权后回调时会原样带回。|
  示例：
  https://xmax.io/oauth/authorize?response_type=code&app_id= [YOUR APPID]&redirect_uri=[YOUR_REDIRECT_URI]&scope=[THE_SCOPE] 如果用户授权后，则成功跳转到指定的redirect_uri，并跟上Authorization Code。 redirect_url?code=cb42e130d1471239a27fca6228094f0e&state=state1



  2.通过code获取Access Token 
  
  请求：POST https://xmax.io/oauth/token 
